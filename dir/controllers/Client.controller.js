...
export const createClientFromChat = async (req, res) => {
    try {
        const { account_id, chat_id, from_messenger, name, phone, description } = req.body;
        const custom_fields = { chat_description: description };
        //@ts-ignore
        const client_id = await client.query('INSERT INTO clients (account_id, from_messenger, name, phone, custom_fields) values ($1, $2, $3, $4, $5) RETURNING id', [account_id, from_messenger, name, phone, custom_fields]);
...
export const createClient = async (req, res) => {
    try {
        ...
        //@ts-ignore
        const newPerson = await client.query('INSERT INTO users (name, surname, email, phone) values ($1, $2, $3, $4) RETURNING *', [name, surname, email, phone]);
    ...
    if (req.usersData) {
        try {
            ...
            for (const user of req.usersData) {
                const customFields = {};
                let name, surname, email, phone, utm_campaign, utm_medium, utm_source, utm_content, utm_term, custom_fields;
                ...
                if (surname)
                    insertFields.surname = surname;
                if (phone)
                    insertFields.phone = phone;
                if (utm_source)
                    insertFields.utm_source = utm_source;
                if (utm_medium)
                    insertFields.utm_medium = utm_medium;
                if (utm_campaign)
                    insertFields.utm_campaign = utm_campaign;
                if (utm_content)
                    insertFields.utm_content = utm_content;
                if (utm_term)
                    insertFields.utm_term = utm_term;
                if (Object.keys(customFields).length > 0) {
                    //@ts-ignore
                    insertFields.custom_fields = customFields;
                }
                const insertQuery = 'INSERT INTO clients (name, surname, email, phone, utm_campaign, utm_medium, utm_source,  custom_fields) ' +
