...
export const startCommandWithParams = async (msg, match) => {
    ...
    try {
        ...
        const parsedParams = (await paramParser(match));
        ...
        if (isUser.rows[0]) {
            ...
            telegramBot.sendMessage(msg.chat.id, `Здравствуйте, вас приветствует поддержка онлайн школы Linnik Fitness. Какой у вас вопрос?)`, 
            //@ts-ignore
            { is_bot_message: true });
        }
    }
...
export const startCommand = async (msg, match) => {
    try {
...
export const createChatFromTg = async (msg, client_id) => {
    const telegram_id = msg.chat.id.toString();
    const avatarUrl = await saveUserAvatar(msg.chat.id);
    const query = `INSERT INTO chats 
            (telegram_id, from_messenger, account_id, client_id, avatar) 
...
export const checkClientForAvailability = async (msg) => {
    const telegram_id = msg.chat.id.toString();
...
export const checkChatForAvailability = async (msg) => {
    const telegram_id = msg.chat.id.toString();
