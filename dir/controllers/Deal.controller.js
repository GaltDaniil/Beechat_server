...
export const getManyDeals = async (req, res) => {
    try {
        ...
        WHERE stage_id = ANY($1)`, [stage_ids]);
        res.status(200).json(result.rows);
    }
    catch (err) {
        await client.query('ROLLBACK');
    ...
    try {
        ...
        if (stage_id && deal_order) {
            ...
            result = await client.query(`
                ...
                WHERE id = $1`, [id, stage_id, deal_order]);
        }
        else if (stage_id) {
...
export const createDealFromGetcourse = async (req, res) => {
    try {
